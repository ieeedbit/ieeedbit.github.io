...
function initializeEvents() {
    ...
    // Load the data
    $.ajax({
        url: "data/yearevents/2024/2024init.json",
        success: function(result) {
            events = result;
            for (myEvent in events) {
                ...
                htmlString += `<a href="/2024/index.html?q=`+myEvent+`" class='name'><strong>` + events[myEvent].title + `</strong></a></div><div class='event-expand'><i class='fa fa-plus'></i></div></div></div>`;
